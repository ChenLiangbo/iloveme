...
import pickle
import cv2
'''本程序使用high,low,open,volume预测当天的close,使用四层神经网络，输入特征数为４'''
class MyNeurolNetworkModel(object):
    ...
    def __init__(self,):
        ...
        self.outdir        = os.path.join(os.path.dirname(__file__),'4predictOne/')             #模型存储路径
...
if __name__ == '__main__':
    ...
    outdir = os.path.join(os.path.dirname(__file__),'4predictOne/')

    myNNmodel = MyNeurolNetworkModel()
    myNNmodel.isdcloseNeeded = False
    myNNmodel.isKdjNeeded = False
    myNNmodel.errorRate = 0.005
