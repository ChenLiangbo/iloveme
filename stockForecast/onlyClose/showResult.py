...
x_real = np.load('./sample/x_sample.npy')
y_real = np.load('./sample/y_sample.npy')
...
y_test_1_real = y_real[test_start:test_end,:]
...
y_test_1_predict = np.load(outdir + 'y_test_1_pridict.npy')
y_test_1_denormalized = y_test_1_predict*(ymax - ymin) + ymin
...
plt.xlabel('index')
plt.ylabel('value')
plt.title('nn4 Only Close Denormalized Training Predict')
...
plt.xlabel('index')
plt.ylabel('value')
plt.title('nn4 Only Close Denormalized Test Predict')
...
plt.title('nn4 Only Close Denormalized Training Predict Acurracy')
...
plt.plot(y_test_1_real,'ro')
plt.plot(y_test_1_denormalized,'bo')
plt.plot(y_test_1_real,'r-')
plt.plot(y_test_1_denormalized,'b-')
plt.xlabel('index')
plt.ylabel('value')
...
y_test_1_acurracy = y_test_1_denormalized - y_test_1_real
shape = y_test_1_acurracy.shape
for i in xrange(shape[0]):
    y_test_1_acurracy[i,0] = (y_test_1_denormalized[i,0] - y_test_1_real[i,0])/y_test_1_real[i,0]
plt.plot(y_test_1_acurracy,'b-')
plt.xlabel('index')
plt.ylabel('y_test_1_acurracy')
plt.title('nn4 Only Close Denormalized Test Predict Acurracy')
plt.legend(['train_acurracy',])
plt.grid(True)
plt.savefig(outdir + 'y_test_1_acurracy.jpg')
plt.show()


print "mean train acurracy = ",np.mean(y_test_1_acurracy)